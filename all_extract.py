...
def save_to_parquet(data, output_path):
    if data:
        arrays = {key: pa.array([item.get(key) for item in data]) for key in DESIRED_KEYS}
...
def process_directory(directory_path, output_filename, checkpoint_dir, process_all):
    ...
    file_counter = 0
    files_processed_in_checkpoint = 0
    ...
    for file in files:
        ...
        # Process the file and add its data
        batch_data = process_file(file_path)
        all_data.extend(batch_data)
        file_counter += 1
        files_processed_in_checkpoint += 1

        print(f"Processed file: {file_path}")

        # Save a checkpoint after processing 2 files
        if files_processed_in_checkpoint == 2:
            ...
            all_data = []  # Reset data after checkpoint is saved
            files_processed_in_checkpoint = 0  # Reset counter for files processed in checkpoint
            print(f"Checkpoint {checkpoint_counter} created at {checkpoint_path}")
        
        # Exit if only processing 3 files and the limit is reached
        if not process_all and file_counter == 3:
            break

    # Save remaining data if any after processing all files
    if all_data:
        final_checkpoint_path = os.path.join(checkpoint_dir, 'final_checkpoint.parquet')
